# genpark-client-side-hydration-runtime-error-triage-skill
...
**genpark-client-side-hydration-runtime-error-triage-skill** is a deterministic, zero-dependency Python skill engineered for autonomous full-stack vibe coding, AST-level frontend component patching, and hydration exception triage.

> **Executive Capability**: React SSR client-side hydration error triager & fix generator (v0.dev / Next)
...
result = client.triage_hydration_error()
print(result)
```
...
```json
{
  "mcpServers": {
    "genpark-client-side-hydration-runtime-error-triage-skill": {
      "command": "python",
      "args": ["/path/to/genpark-client-side-hydration-runtime-error-triage-skill/mcp_server.py"]
    }
  }
}
```
